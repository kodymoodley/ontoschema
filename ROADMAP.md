...
8. Rename object properties to relations, data properties to attributes
9. Support 7+2 design principle (max 9 nodes or classes in one module or graph view), above 9 gets handled with hierarchical decomposition, 7+2 principle on modules so we cap at 81 classes (9 nodes in 9 modules = 9*9 nodes in total). Can still use OntoSchema for larger ontologies by creating multiple 81 node schemas and saving them as separate module files. You can then open them in an editor like Protege later and add links to connect the modules. Regarding edges we cap in-degree for a single node at 5, out-degree at 5 and sum (degree) at 6. And we cap number of edges in total in the schema / module at (num of nodes in schema / module * 1.5). Informative and helpful failure message when graph hits these limits to tell user about possibility to use OntoSchema to create separate schema files for scalability and merging and connecting the separate schema files with a tool like Protege. OntoSchema should also refuse to import schemas and ontologies which violate these limits or extract a user-customised sub-module that respects these limits. Update the example ontologies to respect these criteria too.
10. Import external vocabularies (PROV-O, SKOS, PAV, DCAT), disallow modifying or redefining axioms and definitions from these external vocabularies
11. Add object property edges to taxonomy canvas view
12. Deploy as Github pages site just like I did with my vichara Github project. It is possible to deploy the app as a Github pages site even though the repo is private. You just have to create a corresponding public repo that you can deploy the site and app files to by building the app through Github actions and serving the files in the public repo. You have to make use of some tokens and secrets etc. I have done it before for vichara and vichara-site in my Github repos.
