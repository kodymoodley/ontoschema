...
- **The inspector arc** (todos 26–29) — two changes that between them retire the inspector's
  tab strip, then an entity search. Metadata has left it already; what remains is merging Details
  into Annotations, and then dropping the toggle that only existed to reveal a panel with nothing
  in it. Ordered among themselves and decided; waiting on a
  slot rather than on an answer. **Since neither sequenced item is ready, this is the work that
  can actually be picked up today.** The three before it — the drifted delete labels, the bin
  icon, and the metadata panel — are done.
...
**Done:** 1, 2, 4, 5, 6, 7, 8, 12, 13, 14, 15, 16, 17, 19, 21, 22, 23, 24, 25 — kept as numbers so a commit message naming one can still be resolved.
