...
| **Move ontology metadata out of the inspector, as a panel of its own** — the _Ontology_ tab has the same fault _Export_ had: it describes the document, not the selection, so clicking a class throws you off it. Rename it **Metadata** and give it a trigger. **Where is the open question.** The owner's proposal is a popout button beside undo and redo, which are in the canvas toolbar rather than the header. The alternative worth weighing is the **header, beside the project name field**: the IRI, prefix, title and licence are properties of the document, and the project name is already there, so the document-level things would sit together and the canvas toolbar would stay about the canvas. Against that, the canvas toolbar is roomier and always visible. Owner's call. _(todo 25)_                    | S    |
| **Merge Details and Annotations into one scrolling panel** — both describe the selected entity, so splitting them makes you tab back and forth to see one thing. One column, clear section headings, ordinary vertical scroll. Note what this is worth doing **after**: with _Export_ already gone and _Metadata_ leaving under todo 25, merging these two is what retires the inspector's tab strip **entirely** — four items that together turn a panel of tabs into a panel. _(todo 26)_                                                                                                                                                                                                                                                                                                                                       | S–M  |
| **Drop the Inspector toggle; let selecting an entity open the panel** — a button whose only job is to reveal a panel that has nothing to show until something is selected. Selecting a class on the canvas, or a row in the taxonomy tree, is already the gesture that means _tell me about this_. Two things to settle rather than assume: what closes it again — touching the canvas already dismisses the entities drawer, so deselecting is the obvious answer — and what happens to the keyboard route, since the toggle is currently how the panel is reached without a pointer. Depends on todo 25: if metadata still lived in the inspector there would be a reason to open it with nothing selected. _(todo 27)_                                                                                                         | S    |
...
| **Find an entity by name or description** — a search box over labels, `skos:definition` and `dcterms:description`, ranked with BM25; a hit selects the entity and opens the inspector on it. The thing that makes it worth having is the same thing the subschema filter is for: past about thirty classes the taxonomy tree stops being a way to find anything. Two notes. **BM25 already appears on this list** for discovering terms in an imported vocabulary (todo 10) — one ranking implementation should serve both, and doing this one first is the cheaper way to get it right, over a corpus we own. And **where it goes matters more than the ranking**: the header is full, the left panel is already cramped (see the parked subtabs item), so placement is part of the work rather than a detail of it. _(todo 29)_ | M    |
