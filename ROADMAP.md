...
- **Plain words in the interface** (todo 18) — how far past the annotations section it reaches was
  the open question, and **todo 31 answers it**: as far as the app's own nouns, starting with
  _ontology_ becoming _schema_. That instalment is small and ready; the rest is not.
...
| **Let the side panels slide away on desktop** — both panels are fixed on a wide screen, so the canvas gets what is left rather than what it needs. Collapsing either should give that space back, and the state should persist. **Settle this against todo 27 before building it**: that item has selecting an entity open the inspector, which would undo a deliberate collapse the moment you clicked a class. Either the collapse wins until it is undone by hand, or selection wins and the collapse is a temporary thing — and the answer changes what 27 should do. Below the three-column breakpoint the panels are already drawers, so this is desktop only. _(todo 30)_ | S–M  |
...
| **Say _schema_ rather than _ontology_ in the interface** — the app calls its document an ontology, which is the word its users are least likely to have. _Schema_ is what the tool is for and what the README already leads with. The exported RDF keeps `owl:Ontology`, exactly as it kept `owl:ObjectProperty` when relations and attributes were renamed: the vocabulary belongs to OWL, the words on screen belong to whoever is reading them. Small, and it is the **first instalment of todo 18** — it answers the question that item was waiting on, which was how far past the annotations section the plain words reach. The answer is that they reach the app's own nouns. _(todo 31)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | S    |
...
| 18  | Editing workflow       | Plain words by default, with the vocabulary behind a switch for experts       | M    |
...
| 31  | Editing workflow       | Say _schema_ rather than _ontology_ in the interface — first instalment of 18 | S    |

**Done:** 1, 2, 4, 5, 6, 7, 8, 12, 13, 14, 15, 16, 17, 19, 21, 22, 23, 24, 25, 26 — kept as numbers so a commit message naming one can still be resolved.
