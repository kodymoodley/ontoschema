...
- **The inspector arc** (todos 25–29) — the three changes that between them retire the
  inspector's tab strip, then an entity search. Ordered among themselves and decided; waiting on a
  slot rather than on an answer. **Since neither sequenced item is ready, this is the work that
  can actually be picked up today.** The two that came before it — the drifted delete labels and
  the bin icon — are done.
...
**Done:** 1, 2, 4, 5, 6, 7, 8, 12, 13, 14, 15, 16, 17, 19, 21, 22, 23, 24 — kept as numbers so a commit message naming one can still be resolved.
