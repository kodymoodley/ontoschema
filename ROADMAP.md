...
What a finished feature _does_ is described in the [README](README.md). Items here are struck
through rather than deleted when they ship, because most of them taught something the entry did
not know when it was written — what the problem turned out to be, what had to give, what it cost.
Within each theme the open items come first and the finished ones follow.
...
- **The inspector arc** (todos 23–29) — a label bug on two buttons, a delete icon, the three
  changes that between them retire the inspector's tab strip, and an entity search. Ordered among
  themselves and decided; waiting on a slot rather than on an answer. **Since neither sequenced
  item is ready, this is the work that can actually be picked up today.**
...
- **Plain words in the interface** (todo 18) — waits on a decision, not a slot: how far past the
  annotations tab it reaches.
...
| Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Size |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| **Two delete buttons still say "property"** — a **bug**, left by the relations-and-attributes rename. The inspector's delete button reads _Delete property_ for an attribute ([`AttributeDetails.tsx:128`](src/classeditor/AttributeDetails.tsx)) **and** for a relation ([`RelationDetails.tsx:147`](src/relationeditor/RelationDetails.tsx)); only _Delete class_ is right. The taxonomy tree next to it already says _Delete attribute_, so the same screen contradicts itself. Trivial, and the reason the rename's own test pass missed it is worth a moment's thought: nothing asserted on those labels. _(todo 23)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | XS   |
| **The delete button becomes a red bin** — in the inspector, for all three kinds. It is the one destructive control on the panel and it currently looks like every other button on it. An icon says _destructive_ faster than a word does, and it costs the width a full-word button takes in a 160px drawer. Needs a name and a tooltip, the way the taxonomy toolbar's icons do, and it should stay distinguishable from that toolbar's own delete — two controls that remove the same thing from different places must not answer to one name. Do it with todo 23, since both edit the same three buttons. _(todo 24)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | XS   |
| **Move ontology metadata out of the inspector, as a panel of its own** — the _Ontology_ tab has the same fault _Export_ had: it describes the document, not the selection, so clicking a class throws you off it. Rename it **Metadata** and give it a trigger. **Decided: the header, beside the project name field.** The IRI, prefix, title and licence are properties of the document and the project name is already there, so the document-level things sit together and the canvas toolbar stays about the canvas. The proposal it was chosen over — a popout beside undo and redo — had the better of one argument, that the canvas toolbar is roomier and always visible; grouping won. First in the arc below. _(todo 25)_                                                                                                                                                                                                                                                                                                                                                                                    | S    |
| **Merge Details and Annotations into one scrolling panel** — both describe the selected entity, so splitting them makes you tab back and forth to see one thing. One column, clear section headings, ordinary vertical scroll. **Second in the arc**: with _Export_ already gone and _Metadata_ leaving under todo 25, merging these two is what retires the inspector's tab strip **entirely** — four changes that together turn a panel of tabs into a panel. _(todo 26)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | S–M  |
| **Drop the Inspector toggle; let selecting an entity open the panel** — a button whose only job is to reveal a panel that has nothing to show until something is selected. Selecting a class on the canvas, or a row in the taxonomy tree, is already the gesture that means _tell me about this_. Two things to settle rather than assume: what closes it again — touching the canvas already dismisses the entities drawer, so deselecting is the obvious answer — and what happens to the keyboard route, since the toggle is currently how the panel is reached without a pointer. **Last in the arc**, and it depends on todo 25 rather than merely following it: if metadata still lived in the inspector there would be a reason to open the panel with nothing selected, and the toggle would have to stay. _(todo 27)_                                                                                                                                                                                                                                                                                         | S    |
...
| ~~**Export and import a whole workspace**~~ — _done, as the workspace backup (todo 21)._ One action writes every project exactly as it stands and one reads it back; restoring replaces rather than merges, behind a confirmation. The gap this named was real and is the one thing an RDF document cannot carry: a Turtle file is one ontology and a workspace is several. Originally: save/open works per project; there is no way to move all of them between machines in one file. | S    |
...
| ~~17~~ | Export and interop     | ~~Save and open standard RDF rather than a private format~~ — done                        | —    |
...
| ~~22~~ | Export and interop     | ~~Report what an import discarded~~ — done; "save as a copy" does not apply in a browser  | —    |
...
| 26     | Editing workflow       | Merge Details and Annotations into one scrolling panel                                    | S–M  |
| 27     | Editing workflow       | Drop the Inspector toggle; selecting an entity opens the panel                            | S    |
