...
| G4  | S3  | High   | Tests       | `src/ontologymodel/*.ts`                                                                            | The model scores **77.3%** on mutation testing against a 92% line-coverage threshold, with 251 mutants surviving. Partly addressed: the shape reader's untested paths are covered now, and "no coverage" turned out to mean "no _unit_ test".                                                                      | Kill the survivors file by file, worst first — a programme, not a batch | many         | PARTLY |
...
Eight targeted tests moved the model's score by a tenth of a point. That is the honest size of
G4: a programme, not a batch.
