...
| C4  | S3  | High   | Correctness | `src/ontologymodel/identifier.ts:9`                                                                 | **Found while deciding C2.** The "starts with a letter" guard is `/^[A-Za-z_]/`, so a name in any non-Latin script is prefixed with `_`: `日本語クラス` becomes `_日本語クラス`. XML and NCName both allow those letters.                                                                                          | Widen the guard to the NCName start set                            | 1 file       | FIXED  |
...
| G4  | S3  | High   | Tests       | `src/ontologymodel/*.ts`                                                                            | The model scored **77.2%** on mutation testing against a 92% line-coverage threshold. Now **84.0%**, with 101 mutants killed by tests for behaviours nothing was checking, and a `break` threshold at 79 so the score cannot slip back.                                                                            | Done as far as it is worth doing — see the detail                  | many         | FIXED  |
...
| H2  | S3  | High   | Config      | `eslint.config.js:90`                                                                               | **Found while re-running G3's job.** `.stryker-tmp`, a full copy of the repository that mutation testing works in and leaves behind on a crash, was not ignored — so `npm run lint` failed for anyone who had run `npm run mutation` and passed for everyone else.                                                 | Add it to the ignore list                                          | 1 file       | FIXED  |
...
**Done, and the score is worth having.** A run over both pure layers, 1,947 mutants:

| Layer           | Score     | Survived | No _unit_ cover |
| --------------- | --------- | -------- | --------------- |
| `ontologymodel` | **77.3%** | 251      | 79              |
| `serialization` | **70.3%** | 123      | 21              |

Two corrections came out of reading it, and both changed the finding rather than the code.

**"No coverage" does not mean untested.** The run drives the unit tier only, because the
integration tier needs jsdom — adding it makes Stryker's own dry run fail before a single mutant
is tried. So a `NoCoverage` mutant is a line whose only cover is a heavier tier. G4 originally
read 95 of them as untested lines, which was wrong; the config now says so where the next reader
will look.

**Some survivors are unkillable.** Two guards in the shape reader are redundant against a
malformed _value_: delete the check that `sh:targetClass` is an IRI and a literal target still
produces nothing, because the class lookup below rejects it anyway. Those are equivalent mutants,
and a target of 100% would be a target to write tests that assert nothing. What the guards
genuinely protect is a shape with no target or no path at all — which is now tested, and fails
without them.

**Finished, at 84.0%.** Four rounds, thirty-odd tests, 101 mutants killed:

| File             | Before    | After     |
| ---------------- | --------- | --------- |
| `ontology.ts`    | 63.2%     | **88.8%** |
| `identifier.ts`  | 84.7%     | **90.1%** |
| `mutations.ts`   | 76.2%     | **86.1%** |
| `triples.ts`     | 81.5%     | **84.0%** |
| `fromTriples.ts` | 68.9%     | **74.4%** |
| `taxonomy.ts`    | 89.8%     | **90.6%** |
| **the layer**    | **77.2%** | **84.0%** |

What the survivors turned out to be about, since that is the return on the exercise: renaming a
thing to the name it already has; deleting a property that is another's superproperty; two labels
differing only by language tag; a class that is its own superclass; a document whose terms live in
two namespaces; a union naming a class it never declares; a subproperty inheriting its ends from a
grandparent; a subproperty ring that must not be walked for ever. Every one of those is a real
input, and none was covered. `ontology.ts` had no unit test file at all.

**Stopped deliberately at 233 remaining, and the reason is in what they are.** Of them, 49 mutate
the text inside an error message, 18 mutate a `?? []` behind a lookup that cannot miss, and a
large share of the rest are equivalent — the guard whose absence is caught by the next line
anyway. Killing those means asserting on strings nobody reads and on branches nothing can reach,
which is how a suite full of tests that assert nothing gets written. The number is now held by a
`break` threshold at 79 in `stryker.config.json`: a ratchet against regression rather than a
target to climb.
