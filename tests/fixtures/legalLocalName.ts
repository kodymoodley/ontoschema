...
export function isLegalLocalName(name: string): boolean {
  ...
  // XML's NameStartChar without the colon: the same production the code implements. Stating a
  // spec twice is not the duplication this oracle exists to avoid — what it must not share is
  // the *implementation*, and a range list read off the standard is not that.
  return /^[A-Z_a-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\u{10000}-\u{EFFFF}]/u.test(
    name,
  );
}
